...
async def _extract_zip_from_path(temp_zip_path: Path, extract_to: Path):
    ...
    try:
        ...
        logger.error(f"Failed to extract zip file {temp_zip_path.name}: {e}", exc_info=True)
...
async def upload_send_study_zip(
    ...
    file: UploadFile = File(..., description="Zip archive containing SEND study (.xpt files)")
):
    """
    ...
    try:
        ...
        logger.error(f"Failed to save uploaded file {file.filename} to {temp_zip_path}: {e}")
    ...
    logger.info(f"Accepted upload for study_id: {cleaned_study_id}. Extraction scheduled from {temp_zip_path}")
    return {"message": f"Upload accepted for study '{cleaned_study_id}'. Extraction scheduled.", "study_id": cleaned_study_id}
...
async def analyze_study_with_txgemma(study_id: str):
    ...
    if not study_dir.exists() or not any(study_dir.iterdir()):
        logger.error(f"Study data directory not found or empty for study_id: {cleaned_study_id} at {study_dir}")
        raise HTTPException(status_code=404, detail=f"Study '{cleaned_study_id}' not found or data is missing. Please upload first.")
    ...
    try:
        ...
        if not loaded_domains:
            ...
            raise HTTPException(status_code=500, detail="Failed to load any SEND domains. Directory might be empty or inaccessible.")
        ...
        raise HTTPException(status_code=404, detail=f"Could not find necessary files for study {study_id}. Ensure study is uploaded correctly.")
    except ValueError as e:
        logger.error(f"Data processing error for study {study_id}: {e}")
        raise HTTPException(status_code=400, detail=f"Error processing data for study {study_id}: {e}")
    except Exception as e:
        logger.error(f"Unexpected error during TxGemma analysis pipeline for study {study_id}: {e}", exc_info=True)

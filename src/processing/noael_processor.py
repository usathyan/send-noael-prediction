...
def _extract_numeric_dose(dose_val: Any) -> Optional[float]:
    ...
    try:
        ...
        try:
            ...
            # logger.warning("Exception during regex dose extraction")
            return np.nan
...
def summarize_domain(
    ...
    # Merge dose info
    df_with_dose = pd.merge(
        df, subjects_df[["DOSE_NUM"]], left_on="USUBJID", right_index=True, how="left"
    ...
    try:
        if domain_name == "Clinical Observations (CL)":
            if "CLTERM" in df_with_dose.columns:
                ...
                cl_counts = (
                    df_with_dose[df_with_dose["CLSTRESC"] != "NORMAL"]
                    .groupby("DOSE_NUM")["CLTERM"]
                    .nunique()
                )
                if not cl_counts.empty:
                    for dose, count in cl_counts.items():
                        ...
                        if not pd.isna(control_dose) and dose == control_dose:
                            group_label = f"Control Group ({dose:.2f} {dose_unit_str})"
                        summary_lines.append(
                            f"- {group_label}: {count} distinct non-normal observation(s) recorded."
                        )
                else:
            ...
            if (
                not lb_data.empty
                and "LBSTRESN" in lb_data.columns
                and "LBSTRESU" in lb_data.columns
            ):
                lb_data["LBSTRESN"] = pd.to_numeric(
                    lb_data["LBSTRESN"], errors="coerce"
                )
                lb_data = lb_data.dropna(subset=["LBSTRESN"])
                # Calculate mean value per dose group for key tests
                mean_lb = (
                    lb_data.groupby(["DOSE_NUM", "LBTESTCD"])["LBSTRESN"]
                    .mean()
                    .unstack()
                )
                if not mean_lb.empty:
                    ...
                    for dose, results in mean_lb.iterrows():
                        ...
                        for test, value in results.dropna().items():
                            ...
                            unit = (
                                lb_data.loc[
                                    lb_data["LBTESTCD"] == test, "LBSTRESU"
                                ].iloc[0]
                                if not lb_data.loc[
                                    lb_data["LBTESTCD"] == test, "LBSTRESU"
                                ].empty
                        ...
                        if test_results:
                            summary_lines.append(
                                f"- {group_label}: {'; '.join(test_results)}"
                            )
                else:
                    summary_lines.append("No numeric results for key tests found.")
            else:
                summary_lines.append(
                    "No data for key tests (ALT, AST, BUN, CREA) or relevant columns missing."
                )

        elif domain_name == "Macroscopic Findings (MA)":
            if "MANTERM" in df_with_dose.columns:
                ma_counts = (
                    df_with_dose[df_with_dose["MASTRESC"] != "NORMAL"]
                    .groupby("DOSE_NUM")["MANTERM"]
                    .nunique()
                )
                if not ma_counts.empty:
                    for dose, count in ma_counts.items():
                        ...
                        if not pd.isna(control_dose) and dose == control_dose:
                            group_label = f"Control Group ({dose:.2f} {dose_unit_str})"
                        summary_lines.append(
                            f"- {group_label}: {count} distinct non-normal gross finding(s) recorded."
                        )
                else:
            ...
            if "MITERM" in df_with_dose.columns and "MISEV" in df_with_dose.columns:
                mi_counts = (
                    df_with_dose[df_with_dose["MISTRESC"] != "NORMAL"]
                    .groupby("DOSE_NUM")["USUBJID"]
                    .nunique()
                )
                if not mi_counts.empty:
                    for dose, count in mi_counts.items():
                        ...
                        summary_lines.append(
                            f"- {group_label}: {count} animal(s) with non-normal microscopic finding(s)."
                        )
                else:
                    summary_lines.append("No non-normal microscopic findings recorded.")
            else:
                summary_lines.append(
                    "Relevant columns (e.g., MITERM, MISEV) not found."
                )
            ...
            if (
                not om_data.empty
                and "OMSTRESN" in om_data.columns
                and "OMSTRESU" in om_data.columns
            ):
                om_data["OMSTRESN"] = pd.to_numeric(
                    om_data["OMSTRESN"], errors="coerce"
                )
                om_data = om_data.dropna(subset=["OMSTRESN"])
                mean_om = (
                    om_data.groupby(["DOSE_NUM", "OMTESTCD"])["OMSTRESN"]
                    .mean()
                    .unstack()
                )
                if not mean_om.empty:
                    ...
                    for dose, results in mean_om.iterrows():
                        ...
                        for organ, value in results.dropna().items():
                            unit = (
                                om_data.loc[
                                    om_data["OMTESTCD"] == organ, "OMSTRESU"
                                ].iloc[0]
                                if not om_data.loc[
                                    om_data["OMTESTCD"] == organ, "OMSTRESU"
                                ].empty
                            ...
                            organ_results.append(
                                f"{organ}: {value:.3f} {unit}"
                            )  # Use 3 decimal places for weight
                        if organ_results:
                            summary_lines.append(
                                f"- {group_label}: {'; '.join(organ_results)}"
                            )
                else:
                    summary_lines.append("No numeric results for key organs found.")
            else:
                summary_lines.append(
                    "No data for key organs (LIVER, KIDNEY, SPLEEN) or relevant columns missing."
                )
...
def process_study_for_txgemma(parsed_data: Dict[str, Any], study_id: str) -> Dict:
    ...
    try:
        ...
        logger.info(f"Available domains for analysis: {', '.join(available_domains)}")

        # Check if BW domain is missing but continue with placeholder message
        if bw_df is None or bw_df.empty:
            logger.warning(
                "BW domain data is missing. Will continue with limited analysis."
            )
            bw_analysis_summary = "No Body Weight data available. This study does not contain the BW domain."
        # TS is useful but potentially optional for basic BW analysis
        if not ts_df:
            logger.warning(
                "TS domain data missing or empty. Study metadata might be incomplete."
            )
        ...
        # 3. Perform Body Weight Analysis if available
        if not (bw_df is None or bw_df.empty):
            ...
            if bw_df_clean.empty:
                logger.warning(
                    "No valid numeric Body Weight data found after cleaning."
                )
                return {
                    "study_id": study_id,
                    "status": "Analysis Failed",
                    "analysis_type": "Comprehensive",
                    "comprehensive_findings_summary": "Body Weight data was present but contained no valid numeric measurements.",
                    "llm_prompt": "No prompt generated.",
                    "llm_response": "LLM not called.",
                    "error": "No valid numeric Body Weight data found after cleaning.",
                }
        else:
            logger.info("Skipping Body Weight analysis due to missing BW domain.")
            return {
                "study_id": study_id,
                "status": "Analysis Limited",
                "analysis_type": "Limited",
                "comprehensive_findings_summary": "No Body Weight data available. The study does not contain the BW domain, which is required for standard NOAEL analysis. Try using /analyze_enhanced endpoint instead.",
                "llm_prompt": "No prompt generated.",
                "llm_response": "This study doesn't contain body weight data which is required for standard analysis. Please use the enhanced endpoint (/analyze_enhanced) which can analyze other domains.",
                "error": None,
            }

        # Merge dose info with BW data
        bw_data_with_dose = pd.merge(
            bw_df_clean,
        ...
        baseline_bw = baseline_bw.set_index("USUBJID")[["BWSTRESN"]].rename(
            columns={"BWSTRESN": "BW_BASELINE"}
        )

        bw_data_with_baseline = pd.merge(
            bw_data_with_dose,
            baseline_bw,
            left_on="USUBJID",
            right_index=True,
        ...
        bw_data_with_baseline["BW_PCT_CHANGE"] = (
            (bw_data_with_baseline["BWSTRESN"] - bw_data_with_baseline["BW_BASELINE"])
            / bw_data_with_baseline["BW_BASELINE"]
        ) * 100
        ...
        terminal_bw_change = bw_data_with_baseline[
            bw_data_with_baseline["BWDY"] == terminal_day
        ]

        # Group by DOSE_NUM and calculate mean pct change
        mean_terminal_change_per_dose = (
            terminal_bw_change.groupby("DOSE_NUM")["BW_PCT_CHANGE"].mean().sort_index()
        )
        logger.info(
            f"Mean terminal BW % change by dose:\n{mean_terminal_change_per_dose}"
        )
        ...
        for dose, change in mean_terminal_change_per_dose.items():
            group_label = (
                f"Control Group ({dose:.2f} {dose_unit_str})"
                if not pd.isna(control_dose) and dose == control_dose
                else f"Dose Group ({dose:.2f} {dose_unit_str})"
            )
            bw_analysis_summary_lines.append(
                f"- {group_label}: Mean terminal BW change: {change:.2f}%"
            )
        ...
        all_summaries.append(
            summarize_domain(
                lb_df, "Laboratory Tests (LB)", subjects_df, control_dose, dose_unit_str
            )
        )
        all_summaries.append(
            summarize_domain(
        ...
        comprehensive_findings_summary = "\n".join(
            ...
            if s
            and "No data available." not in s
            and "Summarization logic not implemented" not in s
        )  # Filter out empty/unavailable
        ...
        study_metadata_lines.append(
            f"- Sexes Tested: {', '.join(sex for sex in sexes if pd.notna(sex))}"
        )
        ...
        if ts_df:
            ...
            duration_keys = [
                "TRTDUR",
                "STUDYDAY",
            ]  # Common parameter codes for duration
...
"""
    ...
    results = {
        ...
        "comprehensive_findings_summary": bw_analysis_summary,  # Use BW summary for backwards compatibility
